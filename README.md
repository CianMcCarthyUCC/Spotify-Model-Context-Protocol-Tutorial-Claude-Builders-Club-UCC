...
Step 2 — Install and run the MCP server creator (docs here):
...
Stuck or want to see how we did it? The full implementation is in src/spotifyclaude/server.py. Here's a summary of what each tool does:
Tool	Description
...
When asked which API/SDK you plan to use, select Web API:
Add http://127.0.0.1:8000/callback as a Redirect URI in your app settings.
Copy your Client ID and Client Secret.
Add your Spotify account email under User Management (required while the app is in Development Mode).
...
Debugging
Use the MCP Inspector to test tools without Claude Desktop:
